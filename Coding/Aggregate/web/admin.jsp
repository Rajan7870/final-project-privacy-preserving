<!DOCTYPE html>
<html>
<head>
<title>Privacy Preserving Ranked Multi-Keyword Search for Multiple Data Owners in Cloud Computing</title>
	<meta http-equiv="X-UA-Compatible" content="IE=EmulateIE7; IE=EmulateIE9">
	<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
	<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1.0, user-scalable=no"/>
        <link rel="stylesheet" type="text/css" href="css/style.css" media="all" />
        <link rel="stylesheet" type="text/css" href="css/demo.css" media="all" />
</head>
<body>
<div class="container">
     <div class="clr"></div>
            
    
			<!-- freshdesignweb top bar -->
            <div class="freshdesignweb-top">
                <a href="index.html">Home</a>
                <span class="right">
                    <a href="http://www.freshdesignweb.com/beautiful-registration-form-with-html5-and-css3.html">
<!--                        <strong>Back to the freshdesignweb Article</strong>-->
                    </a>
                </span>
                <div class="clr"></div>
                 <div class="clr"></div>
                <div style=" background: #2a78f6;font-size: 20px ">
        <h1 style="color: #ffffff; height: 50px" align="center" >Privacy Preserving Ranked Multi-Keyword Search for Multiple Data Owners in Cloud Computing</h1>
		
	</div>
            </div><!--/ freshdesignweb top bar -->
			<header>
				<h1> Admin Login Form</h1>
            </header>       
      <div  class="form">
    		<form id="contactform" action="Adminlogin" method="post"> 
    			<p class="contact"><label for="name">Name</label></p> 
    			<input id="name" name="name" placeholder=" name" required="" tabindex="1" type="text"> 	 
                <p class="contact"><label for="password"> password</label></p> 
                <input type="password" id="password" name="password" required=""> <br>
              
                      

            <input class="buttom" name="submit" id="submit" tabindex="5" value="Sign me!" type="submit"> 
              <% String msg=request.getAttribute("msg")!=null?request.getAttribute("msg").toString():""; %>
                        <%=msg%>
   </form> 
</div>      
</div>

</body>
</html>
